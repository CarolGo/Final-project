...
def choose(all_stock, basic_start, basic_end, risk_a, processor, basic_times, stock_num):
    ...
    port = port_fin.Portfolio(all_stock, basic_start, basic_end, risk_a, processor)
    returns = port.get_data()
    ...
    for i in range(len(all_stock)):
        ...
        for j in range(len(all_stock)):
            ...
            temp_port = port_fin.Portfolio(temp, basic_start, basic_end, risk_a, processor)
            # get data from all the testing data
            temp_port.returns = pd.concat(
                [returns.iloc[:, i], returns.iloc[:, j]], join='outer', axis=1)
            max_val = temp_port.plo(0, basic_times)[1]
...
if __name__ == '__main__':
    ...
    all_stock = ['MSFT', 'GE', 'C', 'AMD', 'AAPL']
    basic_start = '2018-03-20'
    basic_end = '2018-04-20'
    risk_a = 5.5
    processor = 4
    basic_times = 10
    stock_num = 3
    ...
    fin_port = port_fin.Portfolio(select_stock, '2016-01-01', '2017-05-20', risk_a, processor)
    fin_port.get_data()
    max_we = fin_port.plo(1, 4000)[0]
    fin_port.print(max_we)
